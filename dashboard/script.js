...
// UI Elements for Manual Trigger
const toggleRecordBtn = document.getElementById('toggle-record-btn');

// API Calls
toggleRecordBtn.addEventListener('click', async () => {
    try {
        const res = await fetch('http://localhost:5000/toggle-recording', { method: 'POST' });
        const data = await res.json();
        if (data.status === 'success') {
            toggleRecordBtn.innerText = data.manual_recording ? 'Stop Recording' : 'Start Recording';
            toggleRecordBtn.classList.toggle('active', data.manual_recording);
            addLog(`Manual recording ${data.manual_recording ? 'Started' : 'Stopped'}`, 'system');
        }
    } catch (e) {
